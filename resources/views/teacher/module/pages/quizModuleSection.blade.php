<div class="card mb-6 quiz-module">
    <div class="card-body">
        <form id="quizForm" action="dashboard/teacher/pembelajaran/quiz/{quiz}" method="post">
            @csrf
            @method('PUT')
...
<div class="modal fade" id="importJsonModal" tabindex="-1" aria-labelledby="importJsonModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <form method="POST" action="{{ route('dashboard.teacher.module.importQuizJson') }}" id="importQuizForm">
      @csrf
...
function loadQuizzes(moduleId, contentId) {
    fetch(`/dashboard/teacher/pembelajaran/module/${moduleId}/content/${contentId}/quiz`)
        .then(response => response.json())
        .then(data => {
...
function updateFormData(quiz) {
    ...
    if(quiz.quiz_id == null){
        ...
        form.action = `/dashboard/teacher/pembelajaran/quiz`;
        form.method = `POST`;
        ...
        form.action = `/dashboard/teacher/pembelajaran/quiz/${quiz.quiz_id}`;
    ...
    showConfirmationModal({
        ...
        onConfirm: function () {
            if (optionId?.startsWith("new-")) {
                ...
                fetch(`/dashboard/teacher/pembelajaran/quiz/option/${optionId}`, {
                    method: "DELETE",
                    headers: {
...
function addNewQuiz() {
    ...
    toggleEditor("multiple_choice");
    document.getElementById('quizForm').action = "/dashboard/teacher/pembelajaran/quiz";
