    ...
    <div class="offcanvas-body my-auto mx-0 flex-grow-0">
        <form id="studentForm" method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col mb-6">
                        <label for="nameUpdate" class="form-label">Full Name</label>
                        <input type="text" id="nameUpdate" name="nameUpdate"
                            class="form-control @error('nameUpdate') is-invalid @enderror" placeholder="Enter name" value="{{ old('nameUpdate') }}"/>
                        @error('nameUpdate')
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                document.getElementById('nameUpdate').value = this.dataset.name || '';
                document.getElementById('NISEdit').value = this.dataset.nis || '';
                document.getElementById('institutionEdit').value = this.dataset.institution || '';
                document.getElementById('addressEdit').value = this.dataset.address || '';
                document.getElementById('emailEdit').value = this.dataset.email || '';
                
                // Jika ingin juga set form action:
                const form = document.getElementById('studentForm');
                const studentId = this.dataset.id;
                form.action = `/dashboard/teacher/users/student/${studentId}`;
            });
        });
    });
</script>